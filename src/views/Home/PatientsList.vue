<template>
  <v-container fluid>
      ...
      <v-card>
        ...
        <v-form lazy-validation>
          ...
          <v-card-actions>
            ...
            <v-btn class="mr-2 ml-4 mb-1" color="error" outlined @click="close">
              Close
            </v-btn>
    ...
    <v-dialog v-model="timeline_dialog"
     ...
     max-width="650px">
      ...
      <v-card  class="mx-auto">
         <v-system-bar
          ...
          style="color: white; font-weight: bold; height: 40px"
        >
          {{patient.firstname}} {{patient.lastname}} Health record
        </v-system-bar>
        ...
        <v-timeline dense v-if=" patient.health_record && patient.health_record.length > 0">
          ...
          <v-card-text class="white text--primary">
            ...
            <div>Temperature Reading: {{record.temperature}} </div>
            <div> Visit date: {{record.visit_date}} </div>
...
export default {
  ...
  methods: {
    ...
    viewHealthRecord(patient_id) {
      ...
      console.log(patient_id);
      axios
        .get(`${config.API_URL}/read_one_patient/` + patient_id)
        .then((response) => {
          if (response.status === 200) {
